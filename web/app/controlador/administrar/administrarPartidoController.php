...
class administrarPartidoController {
    ...
    private function validar() {
        ...
        if ($data['liga_id'] === "" || !is_numeric($data['liga_id'])) {            
            $this->validateMessage = "Debe seleccionar una liga para el partido, si no existe, diríjase al panel de Administración de paises y cree el apropiado";
            return false;
        }
        return true;
    }
    ...
    public function ejecutar(){
        switch ($this->post['evento']){
            case "updateMatchParams":
                $this->m->actualizarParametrosPartido($this->post);                
                break;
        }
        return $this->Response();
    }
